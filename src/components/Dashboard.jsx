...
const Dashboard = ({ user, onLogout }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      setTransactions(transactions.map(t => t.id === formData.id ? transactionData : t));
      setIsEditing(false);
    } else {
  ...
  const getFilteredAndSortedTransactions = () => {
    ...
    if (filterDateRange === '7days') {
      ...
      if (customDateStart) {
        result = result.filter(t => new Date(t.date) >= new Date(customDateStart));
      }
      if (customDateEnd) {
        result = result.filter(t => new Date(t.date) <= new Date(customDateEnd));
      }
    }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-200 font-sans selection:bg-teal-500/30 pb-12 transition-colors duration-300">
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-slate-200 dark:border-slate-700">
                ...
                {userAvatar ? (
                  <img src={userAvatar} alt={userName} className="w-10 h-10 rounded-full object-cover border-2 border-teal-500/50 shadow-sm" />
                ) : (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 animate-fade-in-up">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">{getGreeting()}, {userName}! Here's your financial overview.</p>
        </div>

        {/* Summary Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-teal-500 to-emerald-600 rounded-3xl p-6 shadow-xl shadow-teal-500/20 relative overflow-hidden group transform transition-transform hover:-translate-y-1 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
            ...
            <div className="relative z-10">
              <p className="text-sm font-medium text-teal-100 mb-1">Total Balance</p>
              <h3 className="text-4xl font-bold text-white mb-2">${balance.toFixed(2)}</h3>
            </div>
          </div>

          <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center transform transition-all hover:-translate-y-1 hover:shadow-md animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center transform transition-all hover:-translate-y-1 hover:shadow-md animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Expenses</p>
            <h3 className="text-2xl font-bold text-rose-500 dark:text-rose-400">${totalExpenses.toFixed(2)}</h3>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mt-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm sticky top-28 animate-fade-in-up transition-colors duration-300 hover:shadow-md" style={{ animationDelay: '0.5s' }}>
                ...
                <div>
                  ...
                  <div className="flex gap-4 mb-2">
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <div className={`w-4 h-4 rounded-full border ${formData.type === 'Expense' ? 'border-rose-500 border-[5px]' : 'border-slate-400 group-hover:border-rose-400'} transition-all`}></div>
                      <input
                        type="radio"
                        name="type"
                        value="Expense"
                        ...
                        className="hidden"
                      />
                      <span className="text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Expense</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <div className={`w-4 h-4 rounded-full border ${formData.type === 'Income' ? 'border-emerald-500 border-[5px]' : 'border-slate-400 group-hover:border-emerald-400'} transition-all`}></div>
                      <input
                        type="radio"
                        name="type"
                        value="Income"
                        ...
                        className="hidden"
                      />
                      <span className="text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Income</span>
                    </label>
                  </div>
                </div>
                <div>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm h-full flex flex-col animate-fade-in-up transition-colors duration-300" style={{ animationDelay: '0.6s' }}>
              ...
              <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                  <h3 className="text-lg font-bold text-slate-800 dark:text-white">Transactions History</h3>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50 flex-grow bg-white dark:bg-transparent">
                {displayedTransactions.length === 0 ? (
                  <div className="p-16 text-center flex flex-col items-center justify-center animate-fade-in-up">
                    ...
                    <p className="text-slate-500 text-sm mt-1">Try adjusting your filters or add a new transaction.</p>
                  </div>
                ) : (
                  displayedTransactions.map((transaction, index) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner transition-transform group-hover:scale-110 ${transaction.type === 'Income' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-rose-100 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">
                            ...
                            <span className="text-slate-600 dark:text-slate-300">{transaction.category}</span>
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className={`text-lg font-bold tracking-tight ${transaction.type === 'Income' ? 'text-emerald-500 dark:text-emerald-400' : 'text-rose-500 dark:text-rose-400'}`}>
